...
function App() {
  ...
  const TableWithDiffHighlight = ({ data, origData, title, color }) => {
    ...
    const renderCellContent = (row, col, idx) => {
      ...
      if (cleanVal === null) {
        return (
          <span className={`px-1.5 py-0.5 rounded font-bold font-mono text-[9px] uppercase tracking-wide border ${
            darkMode 
      ...
      if (isModified) {
        return (
          <span 
            className={`px-1.5 py-0.5 rounded font-bold border transition-all cursor-help ${
              darkMode 
    ...
    return (
      <div className={`rounded-2xl shadow-xl border overflow-hidden flex-1 flex flex-col h-full animate-fade-in-up ${
        darkMode ? 'bg-slate-900 border-slate-800/80' : 'bg-white border-slate-200'
      }`}>
        <div className={`p-4 border-b font-extrabold flex items-center justify-between ${
          darkMode ? 'border-slate-800/85 bg-slate-950/20' : 'border-slate-100 bg-slate-50/50'
        } ${color}`}>
          <div className="flex items-center space-x-2"><Database size={15} /> <span>{title}</span></div>
          <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono border ${
            darkMode ? 'bg-white/5 border-slate-700 text-slate-300' : 'bg-slate-200 border-slate-300 text-slate-600'
          }`}>{data.length} rows</span>
        </div>
        <div className="overflow-x-auto flex-1 custom-scrollbar">
          <table className="w-full text-xs text-left">
            <thead className={`text-[10px] uppercase sticky top-0 z-10 border-b select-none ${
              darkMode ? 'text-slate-500 bg-slate-950/70 border-slate-800' : 'text-slate-600 bg-slate-100/80 border-slate-200'
            }`}>
              <tr>
                {columns.map(col => (
                    <th key={col} className={`px-4 py-3 font-bold cursor-pointer transition-colors group ${
                      darkMode ? 'hover:bg-slate-800' : 'hover:bg-slate-200/50'
            ...
            <tbody className={darkMode ? 'text-slate-300' : 'text-slate-700'}>
              {sortedData.map((row, i) => (
                <tr key={i} className={`border-b transition-colors ${
                  darkMode ? 'border-slate-800/40 hover:bg-slate-800/30' : 'border-slate-100 hover:bg-slate-50/50'
                }`}>
                  {columns.map(col => (
  ...
  const StatCard = ({ title, value, subtext, icon: Icon, color }) => (
    <div className={`p-5 rounded-2xl border shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group ${
      darkMode 
  ...
  const ColumnCard = ({ col }) => {
    ...
    return (
        <div className={`p-5 rounded-2xl border transition-all flex flex-col justify-between animate-fade-in-up ${
          darkMode 
            ...
            {hasTopValues ? (
                ...
                <div className={`pt-2 border-t mt-2 ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
                  ...
                  <p className={`font-mono text-[10px] truncate p-1.5 rounded border ${
                    darkMode ? 'text-slate-400 bg-slate-950 border-slate-800' : 'text-slate-600 bg-slate-50 border-slate-200'
                  }`}>{col.sample}</p>
                </div>
            )}
        </div>
  ...
  return (
    <div className={`${data ? 'h-screen overflow-hidden' : 'min-h-screen overflow-x-hidden'} font-sans transition-colors duration-350 flex flex-col md:flex-row relative ${
      darkMode 
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md animate-fade-in">
          <div className={`rounded-3xl shadow-2xl max-w-lg w-full mx-4 overflow-hidden animate-scale-up border ${
            darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
          }`}>
            <div className={`p-5 border-b flex justify-between items-center bg-gradient-to-r ${
              darkMode ? 'from-indigo-950 to-slate-900 border-slate-800' : 'from-indigo-50 to-slate-100 border-slate-200'
            }`}>
              ...
              <button onClick={() => setShowSettingsModal(false)} className={`text-slate-400 hover:text-slate-200 transition-colors p-1 rounded-full border ${
                darkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-200 border-slate-300 hover:text-slate-700'
              }`}>
                <X size={14} />
              </button>
            </div>
            <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className={`w-full text-xs rounded-xl p-2.5 outline-none font-bold border ${
                    darkMode ? 'bg-slate-950 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-700 shadow-sm'
                  }`}
                >
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-[10px] font-mono font-bold px-2.5 py-0.5 rounded-full border ${
                    darkMode ? 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20' : 'text-indigo-700 bg-indigo-50 border-indigo-200'
                  }`}>
              ...
              <div className={`space-y-3 p-4 rounded-xl border ${
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold uppercase transition-colors border ${
                      runTypoCorrection 
                  ...
                  <div className={`pt-2 border-t space-y-2 animate-fade-in-up ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-full ${
                        darkMode ? 'text-slate-300 bg-slate-800' : 'text-slate-600 bg-slate-200'
                      }`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`flex flex-col justify-between p-3 rounded-xl border space-y-2 ${
                  darkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'
                }`}>
                  <div>
                  ...
                  <button 
                    ...
                    className={`w-full py-1.5 rounded-lg text-[10px] font-bold uppercase transition-colors border ${
                      runNumericParsing 
            ...
            <div className={`p-4 border-t flex justify-between gap-3 ${
              ...
              <button 
                ...
                className={`px-4 py-2 text-xs font-bold uppercase rounded-xl transition-colors border ${
                  darkMode ? 'bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300' : 'bg-slate-200 hover:bg-slate-305 border-slate-305 text-slate-600'
                }`}
              >
                Reset
              </button>
              <button 
                ...
                className={`px-6 py-2 text-xs font-bold uppercase rounded-xl shadow-lg transition-colors border ${
                  darkMode ? 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500 text-white' : 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500 text-white'
                }`}
              >
      ...
      {!data && (
        <div className="min-h-screen w-full flex flex-col justify-between z-10 transition-colors duration-300">
          <header className={`backdrop-blur border-b px-6 py-4 flex justify-between items-center sticky top-0 z-10 ${
            darkMode ? 'bg-slate-950/70 border-slate-900' : 'bg-white/80 border-slate-200'
          }`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              <button 
                ...
                className={`flex items-center space-x-2 px-4 py-2 border rounded-xl text-xs font-bold transition-colors shadow-sm ${
                  darkMode 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
              ...
              <div className="lg:col-span-6 space-y-8 text-left animate-fade-in-up">
                <div className="space-y-4">
                  <div className={`inline-flex items-center space-x-2 px-3 py-1 text-[10px] font-bold rounded-full shadow-sm border ${
                    darkMode ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400' : 'bg-indigo-50 border-indigo-100 text-indigo-700'
                  }`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={i} className={`p-4 rounded-2xl border transition-all hover:shadow-md ${
                      darkMode ? 'bg-slate-900/40 border-slate-800 hover:border-slate-700' : 'bg-white border-slate-200'
                    }`}>
              ...
              <div className="lg:col-span-6 w-full animate-fade-in-up">
                <div className="relative group max-w-xl mx-auto">
                  ...
                  <div className={`relative p-6 md:p-8 rounded-3xl shadow-2xl border space-y-6 ${
                    darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                  }`}>
                    ...
                    {!file ? (
                      /* Drag Zone */
                      <div 
                        className={`border-2 border-dashed rounded-2xl p-12 flex flex-col items-center justify-center space-y-4 hover:border-indigo-500/55 transition-all cursor-pointer relative overflow-hidden group/zone ${
                          darkMode ? 'border-slate-800 bg-slate-950/40 hover:bg-indigo-900/20' : 'border-slate-300 bg-slate-50/50 hover:bg-indigo-50/20'
                        }`} 
                        onClick={() => document.getElementById('file-upload').click()}
                      >
                        <input id="file-upload" type="file" onChange={(e) => setFile(e.target.files[0])} accept=".csv,.xlsx" className="hidden"/>
                        <div className={`p-4 rounded-full border transition-transform mb-2 shadow-inner ${
                          darkMode ? 'bg-slate-900 border-slate-800 text-indigo-400 group-hover/zone:scale-110' : 'bg-white border-slate-200 text-indigo-500 group-hover/zone:scale-110'
                        }`}><Upload size={24} /></div>
                      ...
                      <div className={`p-5 rounded-2xl text-left space-y-4 border animate-scale-up ${
                        darkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'
                      }`}>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-3">
                            <div className={`p-2.5 rounded-xl border ${
                              darkMode ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20' : 'bg-indigo-50 text-indigo-600 border-indigo-200'
                          ...
                          <button onClick={() => setFile(null)} className={`transition-colors p-1 rounded-full border ${
                            darkMode ? 'text-slate-500 hover:text-rose-400 bg-slate-900 border-slate-800' : 'text-slate-400 hover:text-rose-600 bg-white border-slate-200'
                          }`} title="Remove File">
                            <X size={14} />
                          </button>
                        </div>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-slate-400 font-medium text-[11px]">
                            {[
                              ...
                              <div key={idx} className={`flex justify-between items-center p-2 rounded-lg border ${
                                darkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-white border-slate-100 text-slate-700'
                              }`}>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      <button 
                        ...
                        className={`flex items-center space-x-2 px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide transition-all border w-full sm:w-auto justify-center ${
                          maskPii 
                      ...
                      <button 
                        ...
                        className={`w-full sm:w-auto text-white px-8 py-3 rounded-xl font-bold transition-all shadow-lg disabled:opacity-50 disabled:shadow-none flex items-center justify-center border ${
                          darkMode ? 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500' : 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500'
                        }`}
                      >
          ...
          <footer className={`py-4 border-t text-center text-xs text-slate-500 ${
            darkMode ? 'border-slate-900 bg-slate-950' : 'border-slate-200 bg-slate-100/50'
          }`}>
      ...
      {data && (
        <div className="flex flex-col md:flex-row w-full h-screen overflow-hidden z-10 transition-colors duration-300">
          ...
          <aside className={`w-full md:w-64 flex flex-col justify-between border-r shrink-0 h-auto md:h-full ${
            darkMode ? 'bg-slate-950 border-slate-900/60 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-600'
          }`}>
            <div className="flex flex-col">
              {/* Sidebar Header Brand */}
              <div className={`p-6 border-b flex items-center space-x-3 cursor-pointer ${
                darkMode ? 'border-slate-900 hover:bg-slate-900/30' : 'border-slate-200 hover:bg-slate-200/50'
              }`} onClick={handleReset}>
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl text-xs font-bold tracking-wide transition-all border ${
                        isActive 
            ...
            <div className={`p-4 border-t space-y-2 ${
              darkMode ? 'border-slate-900 bg-slate-950/70' : 'border-slate-200 bg-slate-50/60'
            }`}>
              {/* Sun/Moon Theme Toggler */}
              <button 
                onClick={() => setDarkMode(!darkMode)} 
                className={`w-full flex items-center justify-center space-x-2 px-3 py-2 rounded-xl text-xs font-bold tracking-wide transition-colors border ${
                  darkMode 
              ...
              <button 
                onClick={() => setShowSettingsModal(true)} 
                className={`w-full flex items-center justify-center space-x-2 px-3 py-2 rounded-xl text-xs font-bold tracking-wide transition-colors border ${
                  darkMode ? 'bg-slate-900 hover:bg-slate-800 text-slate-200 border-slate-800' : 'bg-white hover:bg-slate-100 text-slate-700 border-slate-200 shadow-sm'
                }`}
              >
                <Filter size={13} />
          ...
          <main className={`flex-1 flex flex-col h-full overflow-hidden ${
            darkMode ? 'bg-slate-950/20' : 'bg-slate-50/20'
          }`}>
            
            {/* Top Workspace Header */}
            <header className={`border-b px-6 py-4 flex flex-col lg:flex-row justify-between items-center gap-4 shrink-0 ${
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${
                    darkMode ? 'bg-slate-900 text-slate-400 border-slate-800' : 'bg-slate-200 text-slate-650 border-slate-300'
                  }`}>{file?.name}</span>
              ...
              <div className="relative w-full lg:w-96 group">
                ...
                <input 
                  ...
                  className={`block w-full pl-8 pr-8 py-2 border rounded-xl text-xs font-semibold tracking-wide transition-all outline-none ${
                    darkMode 
                ...
                {aiError && (
                  <div className={`absolute top-full left-0 right-0 mt-1.5 p-2 text-white text-[10px] font-semibold rounded-lg shadow-lg z-25 text-center border animate-fade-in-up ${
                    darkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-950 border-slate-800'
                  }`}>
                    {aiError}
                  </div>
                )}
            ...
            <div className="p-6 md:p-8 flex-1 overflow-y-auto min-h-0 custom-scrollbar">
              ...
              {activeTab === 'dashboard' && (
                <div className="space-y-6 animate-scale-up">
                  ...
                  <div className={`p-6 md:p-8 rounded-3xl border shadow-xl relative overflow-hidden flex flex-col md:flex-row justify-between items-center gap-6 ${
                    darkMode 
                    ...
                    <div className="space-y-2 z-10 text-center md:text-left">
                      <span className={`text-[9px] border px-2.5 py-0.5 rounded-full font-extrabold uppercase tracking-widest font-mono ${
                        darkMode ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400' : 'bg-indigo-100 border-indigo-200 text-indigo-700'
                    ...
                    {/* Health index card */}
                    <div className={`p-4 rounded-2xl border flex flex-col justify-center items-center shrink-0 w-full md:w-56 ${
                      darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                    }`}>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className={`p-6 rounded-2xl shadow-md border flex flex-col justify-between space-y-6 ${
                    ...
                    <div className={`p-6 rounded-2xl shadow-md border flex flex-col justify-between space-y-6 ${
                        ...
                        {data.insights.anomaly_list && data.insights.anomaly_list.length > 0 ? (
                          <div className="space-y-2 max-h-[160px] overflow-y-auto custom-scrollbar pr-1">
                            {data.insights.anomaly_list.map((anom, idx) => (
                              <div key={idx} className={`p-2.5 rounded-xl border text-[11px] font-medium leading-relaxed ${
                      ...
                      <div className={`p-3 rounded-xl border text-xs leading-relaxed flex items-start space-x-2 ${
                        darkMode ? 'bg-indigo-500/5 border-indigo-500/10 text-indigo-300' : 'bg-indigo-50/50 border-indigo-100 text-indigo-900'
                      }`}>
                    ...
                    <div className={`p-6 rounded-2xl border shadow-md flex flex-col space-y-6 ${
                      darkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'
                    }`}>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className={`rounded-xl p-3 h-28 overflow-y-auto text-[9px] font-mono space-y-1.5 border custom-scrollbar select-all ${
                          darkMode ? 'bg-slate-950 text-slate-400 border-slate-800' : 'bg-slate-950 text-slate-400 border-slate-900'
                        }`}>
              ...
              {activeTab === 'viewer' && (
                <div className="space-y-4 animate-scale-up flex flex-col h-full">
                  ...
                  <div className={`p-4 border rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 ${
                    darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                  }`}>
                    ...
                    <div className={`flex items-center space-x-2 p-1 rounded-xl border ${
                      darkMode ? 'bg-slate-950/20 border-slate-800/10' : 'bg-slate-100 border-slate-200 shadow-sm'
                    }`}>
                      <button 
                    ...
                    <div className="space-y-3 flex-1 flex flex-col h-[500px]">
                      <div className="flex justify-start space-x-2">
                        <button 
                          ...
                          className={`px-4 py-2 border text-xs font-bold uppercase rounded-xl transition-all ${
                            viewerTab === 'cleaned' 
                              ? 'bg-indigo-600 border-indigo-500 text-white shadow-md' 
              ...
              {activeTab === 'profiler' && (
                <div className="space-y-6 animate-scale-up">
                  ...
                  <div className={`p-5 border rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6 ${
                    darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                  }`}>
                    <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <select 
                        ...
                        className={`border text-xs rounded-xl p-2 outline-none font-bold ${
                          darkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'border-slate-200 bg-white text-slate-700 shadow-sm'
                        }`}
                      >
                ...
                <div className="space-y-6 animate-scale-up">
                  <div className={`p-6 rounded-2xl border shadow-md ${
                    ...
                    <div className={`flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 border-b pb-4 ${
                      ...
                      <div className="flex flex-wrap items-center gap-3 bg-slate-950/20 p-1.5 rounded-xl border border-slate-800/10">
                        ...
                        <select 
                          ...
                          className={`border text-[11px] rounded-lg p-2 outline-none font-bold ${
                        ...
                        {chartType === 'Scatter' && (
                          <>
                            ...
                            <select 
                              ...
                              className={`border text-[11px] rounded-lg p-2 outline-none font-bold ${
                                darkMode ? 'bg-slate-900 border-slate-800 text-slate-305' : 'bg-white border-slate-200 text-slate-700 shadow-sm'
                              }`}
                            >
                              {data.insights.numeric_columns.map(c => <option key={c} value={c}>{c}</option>)}
                            </select>
                    ...
                    <div className="flex flex-col lg:flex-row gap-6 items-stretch">
                      ...
                      {activeColumnStats && (
                        <div className={`p-4 rounded-xl border flex flex-col justify-center min-w-[200px] ${
                          darkMode ? 'bg-slate-950 border-slate-800 text-slate-350 font-mono' : 'bg-slate-50 border-slate-200 text-slate-700 font-mono'
                        }`}>
              ...
              {activeTab === 'sql' && (
                <div className={`rounded-3xl shadow-xl overflow-hidden border flex flex-col h-[520px] animate-scale-up ${
                  darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'
                }`}>
                  <div className={`flex justify-between items-center px-6 py-3 border-b ${
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <select 
                        ...
                        className={`text-xs rounded-lg p-1.5 outline-none font-bold border ${
                          darkMode ? 'bg-slate-900 border-slate-800 text-slate-300' : 'bg-white border-slate-200 text-slate-700 shadow-sm'
                        }`}
                      >
                    ...
                    <button 
                      ...
                      className={`flex items-center space-x-2 px-4 py-2 text-xs font-bold uppercase rounded-xl transition-all border ${
                        darkMode ? 'bg-indigo-600 hover:bg-indigo-700 border-indigo-500 text-white' : 'bg-indigo-600 hover:bg-indigo-750 border-indigo-500 text-white shadow-indigo-100'
                      }`}
                    >
                      <Copy size={13} /> 
            ...
            <footer className={`border-t px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 shrink-0 ${
              darkMode ? 'bg-slate-950 border-slate-900' : 'bg-white border-slate-200'
            }`}>
              <div>
              ...
              <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
                <button 
                  ...
                  className={`flex items-center space-x-1.5 px-4 py-2 text-xs font-bold rounded-xl transition-colors border shadow-md ${
                ...
                <button 
                  ...
                  className={`flex items-center space-x-1.5 px-4 py-2 border rounded-xl text-xs font-bold transition-colors ${
                    darkMode ? 'bg-slate-900 border-slate-800 text-slate-300 hover:bg-slate-800' : 'bg-white border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm'
                  }`}
                >
                  <Code size={14} /> 
                ...
                <button 
                  ...
                  className={`flex items-center space-x-1.5 px-4 py-2 border rounded-xl text-xs font-bold transition-colors ${
                    darkMode ? 'border-rose-500/20 text-rose-400 hover:bg-rose-500/10' : 'border-rose-300 text-rose-700 hover:bg-rose-50'
                  }`}
                >
                  <PdfIcon size={14} /> 
